...
class TestGpAnova(unittest.TestCase):
    ...
    def test_f0(self):
        ...
        self.assertLess(relative_error(f0_actual, f0_expected), 0.02)
    ...
    def test_total_variance(self):
        ...
        X = np.random.uniform(
            size=self.dim*self.mc_samples).reshape((-1, self.dim))
        tv_expected = np.mean(
            (self.GpAnova.kernel_eval(X) - self.GpAnova.f0())**2)
        self.assertLess(relative_error(tv_actual, tv_expected), 0.02)

    def test_f_t(self):
        X = np.random.uniform(size=self.mc_samples *
                              self.dim).reshape((-1, self.dim))
        ...
        for d in range(self.dim):
            ...
            self.assertLess(relative_error(f_t_actual, f_t_expected), 0.02,
    ...
    def test_main_effects(self):
        ...
        for d in range(self.dim):
            ...
            self.assertLess(relative_error(
                main_effect_actual, main_effect_expected), 0.02)
